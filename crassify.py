...
def to_phylip(df_dist, ictv_dists):
    ...
    with Progress() as pb:
        ...
        df_dist['qseqid_virus'] = df_dist['qseqid_ID']
        dists = pd.concat([ictv_dists, df_dist])
        matrix = dists.pivot_table(columns='qseqid_ID', index='sseqid_genome_ncbi_acc', values='distance').reset_index()
        matrix = matrix.set_index('sseqid_genome_ncbi_acc')
        ...
        matrix = matrix.fillna(1) \
            ...
            .round(5) \
            .clip(lower = 0)
        matrix.to_csv(f'{outpath}/phylip.dist', header=True, index=True, sep =' ')
        pb.update(task_id=t1, advance=25)
    return matrix
...
if __name__ == "__main__":
    ...
    phylip = to_phylip(dists, '/home/administrator/phd/crassify/databases/ictv_distances.csv')
